...
void Protocol::run_protocol() {
    ...
    while (TRUE) {
        std::string cmd = client->receive(4);
        
        if (cmd == "ENDP" || attempts == MAX_ATTEMTPS) {
            client->send("ENDP");
            break;
        }
        ...
        if (cmd == "CALC") {
            ...
            int msg_length = 3 + 1 + MESSAGE_LENGTH + 1 + MESSAGE_LENGTH;
            std::string statement = client->receive(msg_length);
            
            if (statement.length() == msg_length) {
